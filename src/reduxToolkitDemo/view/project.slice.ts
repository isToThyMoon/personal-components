 ...
 * @LastEditors: 王荣
 * @LastEditTime: 2022-02-25 13:42:31
 * @Description: 填写简介
 */
...
export const projectSlice = createSlice({
    name: 'projectSlice',// 命名空间，在调用action的时候会默认的设置为action的前缀 自动的把每一个action进行独立，解决了action的type出现同名的文件。在使用的时候默认会把使用name/actionName
    initialState,   //该切片维护的状态
    reducers:{  //定义action。由于内置了immutable插件，可以直接使用赋值的方式进行数据的改变，不需要每一次都返回一个新的state数据。
        openProjectModal(state, action:PayloadAction<boolean>){ // 这里的属性会自动的导出为actions，在组件中可以直接通过dispatch进行触发  
            state.projectModalOpen = true;
        },
        closeProjectModal(state, action:PayloadAction<boolean>){ // action属性推导定义为{payload:any, type: string}
...
export default projectSlice.reducer; // 默认导出reducer 在store中combine
